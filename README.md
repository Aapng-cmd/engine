...
В **редакторе** (панель Physics):

| Режим гравитации | Поведение |
|------------------|-----------|
| **Off** | Нет силы тяжести; объект остаётся на месте (кроме столкновений) |
| **Primitive** | Постоянный вектор ускорения (обычно `gy = -9.81`) |
| **Advanced** | Притяжение к точке или к центру другого объекта (master–slave) |

Параметр **Opacity / reflect (0–2)**: `0…1` — только прозрачность; `1…2` — отражение (зеркало при `2`).
...
Тесты запускаются **по одному** в порядке: `collision_suite` → `pair_suite` → `fourd_suite` → `stress_suite` → `k_axis_suite`. При падении — исправление кода и повтор **того же** теста.
