...
void FourDWireFigure::drawProjected(const Camera4DState& cam, double kWorld) const
{
    glColor4d(color.x, color.y, color.z, renderAlpha);
    glLineWidth(1.5f);
    glBegin(GL_LINES);
    for (const Edge4D& e : edges4) {
        const Vec4 wa = fourd::transformLocal4D(e.a, pos, scale, rx, ry, rz, kWorld);
        const Vec4 wb = fourd::transformLocal4D(e.b, pos, scale, rx, ry, rz, kWorld);
        vec<> a3, b3;
